...
def gaussian_process_regression(t_sampled, y_sampled, t_star, R, sigma_n=0):
    ...
    # Compute GP estimate
    y_hat_star = k_star.T @ np.linalg.inv(R_sampled + sigma_n * np.eye(M)) @ y_sampled
    sigma = R[t_star, t_star] - k_star.T @ np.linalg.inv(R_sampled) @ k_star  # Variance

    return y_hat_star[0], np.sqrt(sigma)
